...
const StatCards = ({ strategies }: StatCardsProps) => {
  ...
  const confChartData = [
    { name: "Guadagnato", value: stats.fatturatoConfermato, color: "hsl(152, 69%, 40%)" },
    { name: "Potenziale", value: stats.fatturatoPotenziale, color: "hsl(220, 14%, 85%)" },
  ];

  const socialSitoData = [
    { name: "Social", value: stats.socialCount, color: "hsl(152, 69%, 40%)" },
    { name: "Sito", value: stats.sitoCount, color: "hsl(262, 52%, 56%)" },
  ];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div className="liquid-card animate-fade-in">
        ...
        <div className="p-6 pt-0 space-y-3">
          <div>
            <span className="block text-2xl sm:text-3xl font-bold font-mono text-foreground tracking-tight">
          ...
          <div>
            <button
              ...
              className="flex items-center justify-between w-full border-t border-border/40 pt-2 group cursor-pointer"
            >
              <p className="text-xs font-semibold text-muted-foreground uppercase">
                Dettaglio Fatturate ({stats.fatturate.length})
              </p>
              <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${showFatturate ? "rotate-180" : ""}`} />
            </button>
            {showFatturate && (
                ...
                {fatturateSocial.length > 0 && (
                  <div>
                    ...
                    {fatturateSocial.map((s) => (
                      <div key={s.id} className="flex justify-between text-sm py-1.5 border-t border-border/30">
      ...
      <div className="liquid-card animate-fade-in" style={{ animationDelay: "0.1s" }}>
        <div className="flex flex-col space-y-1.5 p-6 pb-2">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
        ...
        <div className="p-6 pt-0 space-y-3">
          ...
          <div>
            <button
              ...
              className="flex items-center justify-between w-full bg-muted/30 rounded-xl p-3 group cursor-pointer transition-colors hover:bg-muted/50"
            >
              <p className="text-xs font-semibold text-muted-foreground uppercase">
                {stats.inLavorazione.length} strategie in lavorazione
              </p>
              <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${showInLavorazione ? "rotate-180" : ""}`} />
            </button>
            {showInLavorazione && (
              <div className="mt-2 space-y-1 animate-fade-in">
                {Object.entries(stats.statoCounts).map(([stato, count]) => (
                  <div key={stato} className="flex justify-between text-sm py-1.5 border-t border-border/30">
          ...
          <div>
            ...
            >
              ...
              <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${showDettaglioTipo ? "rotate-180" : ""}`} />
            </button>
            {showDettaglioTipo && (
              <div className="mt-2 space-y-1 animate-fade-in">
                <div className="flex justify-between text-sm py-1.5 border-t border-border/30">
      ...
      {/* Social vs Sito */}
      <div className="liquid-card animate-fade-in" style={{ animationDelay: "0.2s" }}>
        <div className="flex flex-col space-y-1.5 p-6 pb-2">
